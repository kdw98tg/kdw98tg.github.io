...
- 컴파일 타임에 관리가 잘 안되는 주석 사용을 지양하고, 코드를 최대한 깔끔하게 짜는것이 철학이었음. 지금까지는 문제가 없었으나, 규모가 꽤 있는 게임 작업을 하니, 언제 짠지 모를 코드들을 해석하는데 시간이 걸렸음. 코드를 설명하는 주석이 아닌, 코드를 짤때의 의도가 담긴 주석 사용을 지향해야겠다 라는 생각을 가짐