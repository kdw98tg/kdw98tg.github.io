...
2. 변수명은 불필요한 약어 사용을 지양하고, 코드가 자연스럽게 읽히도록 목적과 의미를 충분히 풀어쓴다.
3. 자료구조를 사용할때는, 변수 뒤에 자료구조의 형식을 붙인다.
4. 주석은 무엇에 대한 내용이 아닌, 코드를 작성한 의도를 표현하는 용도로 사용한다.
5. 특별한 이유가 있지 않으면 IDE에서 제안하는 서식을 따른다.
6. 더 좋은 방법이 있을 시 건의하여 코드 컨벤션을 수정한다.
...
### 27. 디폴트 매개변수 대신 함수 오버로딩을 선호한다. 단, 필요 이상으로 오버로드 함수가 많아지면, 예외를 적용한다.
- 틀린 예
