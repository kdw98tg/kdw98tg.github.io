...
React라는 Framework 가 나온 배경에는 여러가지 이유가 있겠지만, 그중, 정적인 html의 렌더링 과정을 동적으로 구현하기 위함도 있습니다. 이 과정을 다시 그린다 즉, 리렌더링 이라고 부릅니다.
...
`UseEffect`는 컴포넌트가 렌더링될 때마다, 특정한 작업을 수행하도록 설정하는 Hook 입니다.
